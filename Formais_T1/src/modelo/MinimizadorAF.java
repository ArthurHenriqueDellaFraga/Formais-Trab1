...
public class MinimizadorAF {

	private static MinimizadorAF instancia;

	private MinimizadorAF() {
	}

	public static MinimizadorAF getInstance() {
		if (instancia == null)
			instancia = new MinimizadorAF();
	...
	/*
	 * MINIMIZACAO DE UM AUTOMATO Elimina primeiramente os estados
	 * inalcancaveis, depois os mortos e por fim simplifica-o.
	 */
	public AutomatoFinito minimizar(AutomatoFinito automato) {
		AutomatoFinito automatoAlcancavel = excluirEstadosInalcancaveis(new AutomatoFinito(
				automato));
		AutomatoFinito automatoVivo = excluirEstadosMortos(new AutomatoFinito(
				automatoAlcancavel));
		AutomatoFinito automatoSimples = simplificar(new AutomatoFinito(
				automatoVivo));
	...
	/*
	 * SIMPLICACAO DE UM AUTOMATO Construcao de um novo automato eliminando os
	...
	/*
	 * DETERMINACAO DE CLASSES DE EQUIVALENCIA Verificao e criacao das classes
	 * equivalentes entre estados.
	 */
	...
	/*
	 * DETERMINACAO DE EQUIVALENCIA ENTRE DOIS ESTADOS Verificao da equivalencia
	 * entre dois estados em um automato.
	 */
	...
	/*
	 * EXCLUSAO DE ESTADOS INALCANCAVEIS Exclusao de estados inalcancaveis do
	 * automato.
	 */
	private AutomatoFinito excluirEstadosInalcancaveis(AutomatoFinito automato) {
		Set<String> estadoInalcancavel = new HashSet<String>(
				determinarEstadosInalcancaveis(automato));
		...
		while (estadoIterator.hasNext()) {
			...
			if (estadoInalcancavel.contains(atual)) {
				Iterator<String> alfabetoIterator = automato.alfabeto
						.iterator();
				while (alfabetoIterator.hasNext()) {
					automato.tabelaDeTransicao.remove(new Transicao(atual,
							alfabetoIterator.next()).hashMap());
				}
			}
		}
	...
	/*
	 * DETERMINACAO DE ESTADOS INALCANCAVEIS Determinacao dos estados nao
	 * alcancaveis de um automato.
	 */
	private Set<String> determinarEstadosInalcancaveis(AutomatoFinito automato) {
		...
		while (alcancadoNovosEstados) {
			...
			Iterator<String> estadoParaVerificarIterator = estadoParaVerificar
					.iterator();
			while (estadoParaVerificarIterator.hasNext()) {
				String atual = estadoParaVerificarIterator.next();
				estadoAlcancado.addAll(automato
						.determinarEstadosDescendentes(atual));
	...
	/*
	 * EXCLUSAO DE ESTADOS MORTOS Exclusao de estados mortos do automato.
	 */
	private AutomatoFinito excluirEstadosMortos(AutomatoFinito automato) {
		Set<String> estadoMorto = new HashSet<String>(
				determinarEstadosMortos(automato));
		Iterator<String> estadoIterator = automato.estado.iterator();
		...
		while (estadoIterator.hasNext()) {
			...
			if (estadoMorto.contains(atual)) {
				...
				Iterator<String> alfabetoIterator = automato.alfabeto
						.iterator();
				while (alfabetoIterator.hasNext()) {
					Transicao transicao = new Transicao(atual,
							alfabetoIterator.next());
					if (estadoMorto.containsAll(automato.tabelaDeTransicao
							.get(transicao.hashMap()))) {
	...
	/*
	 * DETERMINACAO DE ESTADOS MORTOS Determinacao dos estados que nao sao vivos
	 * de um automato.
	 */
	...
	/*
	 * EXCLUIR TRANSICAO Remove uma transicao de uma tabela de transicao,
	 * substituindo o estado destino para FI.
	 */
	private void excluirTransicao(
			HashMap<String, ArrayList<String>> tabelaDeTransicao,
			Transicao transicao) {
